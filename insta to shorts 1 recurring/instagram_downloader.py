...
MAX_RANDOM_DELAY_SECONDS = 10 * 60
...
def generate_title_with_gpt(video_path: str, description: str) -> str:
        ...
        messages=[
            {
                ...
                "content": [
                    ...
                    },
                    {
                        "type": "text",
                        "text": (
                            f"Instagram Caption: {description}\n\n"
                            "gebe mir hierzu einen kurzen titel fuer youtube shorts "
...
def build_daily_schedule(day) -> list:
    """Erstellt die heutigen Upload-Zeiten mit zufaelligem Delay von 0-600 Sekunden."""
    slots = []
    for hour in TARGET_HOURS:
        base = datetime.combine(day, datetime.min.time()).replace(hour=hour)
        delay = random.randint(0, MAX_RANDOM_DELAY_SECONDS)
        slots.append(base + timedelta(seconds=delay))
    return sorted(slots)


def wait_until(target_time: datetime) -> None:
    """Wartet in kleinen Intervallen bis zum Zielzeitpunkt."""
    while True:
...
def get_top_reel_entry(file_path: str):
    """Liest den obersten nicht-leeren Link aus der Queue-Datei."""
    ...
    for idx, line in enumerate(lines):
        url = line.strip()
        if url:
            return idx, url, lines
    return None, None, lines


def remove_reel_entry(file_path: str, lines: list, idx: int) -> None:
    """Entfernt die verarbeitete Zeile aus der Queue-Datei."""
    updated = lines[:idx] + lines[idx + 1:]
    with open(file_path, "w", encoding="utf-8") as fh:
        fh.writelines(updated)


def process_next_reel(reels_file: str, output_dir: str) -> bool:
    """Verarbeitet genau den obersten Reel-Link aus der Datei."""
    idx, url, lines = get_top_reel_entry(reels_file)
    if url is None:
        print("\nKeine Links in insta_reels.txt gefunden. Termin wird uebersprungen.")
        return False

    if "instagram.com" not in url:
        print("\nWarnung: Oberster Eintrag sieht nicht wie eine Instagram-URL aus.")

    print(f"\nStarte Verarbeitung fuer obersten Link: {url}")
    ...
    if not downloaded:
        raise RuntimeError("Heruntergeladene Datei konnte nicht gefunden werden.")

    final = transform_video(downloaded)

    yt_title = generate_title_with_gpt(final, meta["description"])
    ...
    remove_reel_entry(reels_file, lines, idx)
    print("Verarbeiteter Link wurde aus insta_reels.txt entfernt.")
    return True


def run_scheduler() -> None:
    """Laeuft dauerhaft und postet taeglich um 17/18/19 Uhr (+zufaellig 0-10 min)."""
    script_dir = os.path.dirname(os.path.abspath(__file__))
    output_dir = os.path.join(script_dir, "Downloads")
    reels_file = os.path.join(script_dir, "insta_reels.txt")

    planned_day = datetime.now().date()
    slots = build_daily_schedule(planned_day)

    while True:
        now = datetime.now()

        # Falls der Tag gewechselt hat, neue Tages-Slots aufbauen.
        if now.date() > planned_day:
            planned_day = now.date()
            slots = build_daily_schedule(planned_day)

        # Bereits vergangene Slots verwerfen (z.B. bei Neustart spaet am Abend).
        slots = [s for s in slots if s > now]

        if not slots:
            planned_day = planned_day + timedelta(days=1)
            slots = build_daily_schedule(planned_day)

        next_run = slots[0]
        print(
            f"\nNaechster Upload-Slot: {next_run.strftime('%Y-%m-%d %H:%M:%S')} "
            f"(Delay 0-{MAX_RANDOM_DELAY_SECONDS} Sekunden auf Basis {TARGET_HOURS})"
        )

        try:
            wait_until(next_run)
            process_next_reel(reels_file, output_dir)
        except Exception as e:
            print(f"\nFehler beim Upload-Termin: {e}")
            print("Hinweis: Link bleibt in insta_reels.txt erhalten und wird spaeter erneut versucht.")
...
def main():
    ...
    print("=" * 50)
    print("  Instagram Reel Queue Uploader fuer YouTube Shorts")
    print("=" * 50)

    print("Dieses Programm laeuft dauerhaft.")
    print("Tageweise Upload-Slots: 17:00, 18:00, 19:00 Uhr + Zufall 0-10 Minuten.")
    print("Quelle: oberster Link aus insta_reels.txt (Queue-Prinzip).\n")
